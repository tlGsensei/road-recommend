...
def gru_train(course, device, BATCH_SIZE):
    if course == 'java':
        ...
        path = './data/data_science/data_sci_mb.txt'
    ...
    # wangyuan 310
    # touge 8750
    model = path_env(input_size=16, hidden_size=16, num_layers=3, n_users=8750, emb_size=16, item_emb_path=item_emb_path, device=device)
    # n_items 代表序列最长长度
    model.to(device)
...
def dqn_eval(cfg,env,agent,course,k):
    ...
    if course == 'java':
        ...
        path = './data/c_class/c_class_test_10.csv'
        size = 114
    elif course == 'computer':
        path = './data/computer/computer_test_10.csv'
        size = 307
    elif course == 'data_sci':
        path = './data/data_sci/data_sci_test_10.csv'
        size = 263
    else:
        print('course not supported!')
        return
...
def dqn_train_eval(cfg, env, course, k):
    ...
    if course == 'java':
        ...
        path = './data/data_science/data_sci_train_10.csv'
        size = 263
    else:
        print('course not supported!')
        return
    ...
    for i_ep in tqdm(range(cfg.train_eps)):
        ...
        file = open(course+'_result10.txt','a')
    ...
    file.close()
    agent.save('./saved/ten'+course+'_train_'+str(k))
    #print(all_loss)
    return rewards, ma_rewards,agent
...
# dev = 'cpu'
env = gru_train(course=c_name, device=dev, BATCH_SIZE=32)
configs = DQNConfig()
configs.train_eps=1
...
for i in range(1,11):
    ...
    avg_f = 0
    for _ in range(6):
        rewards,_,agent = dqn_train(cfg=configs, env= env, course=c_name,k=i)
