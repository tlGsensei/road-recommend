...
def user_model_train(course, device, BATCH_SIZE):
    if course == 'java':
    ...
    model = path_env(input_size=16, hidden_size=16, num_layers=3, n_users=310, emb_size=16, item_emb_path=item_emb_path, device=device)
    # n_items 代表序列最长长度
    model.to(device)
...
def dqn_train(cfg, env, course, k):
    print('DQN开始训练!')
    print(f'环境：{cfg.env}, 算法：{cfg.algo}, 设备：{cfg.device}')

    if course == 'java':
        path = './data/java/java_train_same.csv'
        size = 363
    elif course == 'net':
        path = './data/net/net_train_same.csv'
        size = 338
    elif course == 'os':
        path = './data/os/os_train_same.csv'
        size = 496
    else:
        print('course not supported!')
        return
    ...
    env.to(cfg.device)
    agent = DQN(state_dim=16*3, action_dim=size,cfg=cfg)
    ...
    for i_ep in tqdm(range(cfg.train_eps)):
        ...
        for batch_idx, data in enumerate(traindata):
            ...
            if (i_ep+1)%10 == 0:
                print('回合：{}/{}, 奖励：{}'.format(i_ep+1, cfg.train_eps, ep_reward.cpu().tolist()))
            rewards.append(ep_reward.cpu().tolist())
            # save ma_rewards
            '''
            if ma_rewards:
...
def dqn_eval(cfg,env,agent,course,k):
    ...
    if course == 'java':
        path = './data/java/java_test_same.csv'
    elif course == 'net':
        path = './data/net/net_test_same.csv'
    elif course == 'os':
        path = './data/os/os_test_same.csv'
    else:
        print('course not supported!')
        return
...
def dqn_train_eval(cfg, env, course, k):
    print('DQN开始训练!')
    print(f'环境：{cfg.env}, 算法：{cfg.algo}, 设备：{cfg.device}')
    ...
    for i_ep in tqdm(range(cfg.train_eps)):
        ...
        for batch_idx, data in enumerate(traindata):
            ...
            '''
            #print("reward:", reward)
        _,_,_,p,r,f = dqn_eval(configs,env,agent,course=course,k=k)
        file = open(course+'_result.txt','a')
        file.write("路径长度 "+str(k)+" precision: "+str(p)+" recall: "+str(r)+" F1 score: "+str(f)+'\n')
        file.close()
        all_p += p
        all_r += r
        all_f += f
    print('dqn完成训练！')
    print('平均precison： ',all_p,"平均recall: ",all_r,"平均f1: ",all_f)
    file = open(course+'_result.txt','a')
    file.write("路径长度 "+str(k)+'平均precison： '+str(all_p)+"平均recall: "+str(all_r)+"平均f1: "+str(all_f)+'\n')
    file.close()
    agent.save('./saved/'+course+'_train_'+str(k))
    #print(all_loss)
    return rewards, ma_rewards,agent

dev = 'cuda' if torch.cuda.is_available() else 'cpu'
# dev = 'cpu'
env = user_model_train(course='os', device=dev, BATCH_SIZE=32)
configs = DQNConfig()
configs.train_eps=1
...
for i in range(1,11):
    ...
    avg_f = 0
    for _ in range(5):
        rewards,_,agent = dqn_train(cfg=configs, env= env, course='os',k=i)
        _,_,_,p,r,f = dqn_eval(configs,env,agent,course='os',k=i)
        file = open('os_result_5.txt','a')
        ...
        avg_f += f
    file = open('os_result_5.txt','a')
    file.write("路径长度 "+str(i)+' 平均precison： '+str(avg_p/5)+" 平均recall: "+str(avg_r/5)+" 平均f1: "+str(avg_f/5)+'\n')
    file.close()
    # dqn_train_eval(cfg=configs, env= env, course='java',k=i)
